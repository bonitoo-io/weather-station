import { ENDPOINT_ROOT } from './Env';

export const SCAN_NETWORKS_ENDPOINT = ENDPOINT_ROOT + "scanNetworks";
export const LIST_NETWORKS_ENDPOINT = ENDPOINT_ROOT + "listNetworks";
export const WIFI_SETTINGS_ENDPOINT = ENDPOINT_ROOT + "wifiSettings";
export const WIFI_STATUS_ENDPOINT = ENDPOINT_ROOT + "wifiStatus";
export const LIST_SAVED_NETWORKS_ENDPOINT = ENDPOINT_ROOT + "savedNetworks";
export const CONNECT_TO_SAVED_ENDPOINT_PATH = ENDPOINT_ROOT + "connectSaved"
export const CONNECT_STATUS_ENDPOINT_PATH = ENDPOINT_ROOT + "connectStatus"
export const INFLUXDB_SETTINGS_ENDPOINT = ENDPOINT_ROOT + "influxDbSettings";
export const INFLUXDB_VALIDATE_ENDPOINT = ENDPOINT_ROOT + "validateInfluxDBParams";
export const ABOUT_INFO_ENDPOINT = ENDPOINT_ROOT + "aboutInfo";
export const RESTART_ENDPOINT = ENDPOINT_ROOT + "restart";
export const FACTORY_RESET_ENDPOINT = ENDPOINT_ROOT + "factoryReset";
export const REGIONAL_SETTINGS_ENDPOINT = ENDPOINT_ROOT + "regionalSettings";
export const REGIONAL_SETTINGS_VALIDATE_ENDPOINT = ENDPOINT_ROOT + "regionalSettingsCheckValidation";
export const UPLOAD_FIRMWARE_ENDPOINT = ENDPOINT_ROOT + "uploadFirmware";
export const ADVANCED_SETTINGS_ENDPOINT = ENDPOINT_ROOT + "advancedSettings";
export const ADVANCED_SETTINGS_VALIDATE_ENDPOINT  = ENDPOINT_ROOT + "validateAdvancedSettings"
export const DISPLAY_SETTINGS_ENDPOINT = ENDPOINT_ROOT + "displaySettings";