...
void drawAstronomy(OLEDDisplay *display, OLEDDisplayUiState* state, int16_t x, int16_t y) {
  ...
  display->drawString(x + 43, y + 13, strTime(currentWeather.sunrise, true) + "-" + strTime(currentWeather.sunset, true));
  display->drawString(x + 40, y + 21, "Moon");
  display->drawString(x + 43, y + 30, strTime(moonRise, true) + "-" + strTime(moonSet, true));
