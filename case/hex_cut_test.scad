...
module shellCriticalPlaces (dist= 2){
  ...
  translate([ -20.5 - dist, 2 - dist, -0.7 ]) 
    cube([ 25 + dist * 2, 14 + dist * 2, 10 ]);
  ...
  translate([0, 0, -1])
  difference() {
    cube_center_xy([ w + shell_width * 2 + 2, d + shell_width * 2 + 2, h + 1 - 2 + 2 + 2 ]);
    translate([ 0, 0, 1.5 + dist ]){
      cube_center_xy([ w+10, d-dist*2, h-dist * 3 ]);
      cube_center_xy([ w-dist*2, d+10, h-dist * 3 ]);
    }
    translate([0,0,-1])
      cube_center_xy([ w-dist*2, d-dist*2, h+1.5+10 ]);
  }

  // main board holder
  translate([ -w / 2 + main_board_width / 2 + 1, 0, h - 10 ])
  difference() {
    cube_center_xy([ main_board_width + 2 + dist * 2, main_board_depth + 2 + dist * 2 + 2*2, 10 ]);
  
    translate ([0,0,-1]) {
      cube_center_xy([ main_board_width + 2 - dist * 2 - 9, main_board_depth + 2 + dist * 2 + 2*2 + 2, 20 + 2 ]);
  ...
  translate([ 18 - dist, -d / 2, -1 ])
    cube([ sensor_wall_width + dist * 2, 28 + dist, 1.5+2 ]);

  translate([ -32, -0.8 - dist, 22.3 - dist ]) 
    cube([ 5, 9.5 + dist * 2, 3.8 + dist * 2 ]);
}
...
module hex_cuts() {
  difference() {
    ...
    union() {
      translate([-3, -6, -.2])
        hex_wall(4, h= 1.5 + 1, rows= 9);

      translate([.5,0,0])
      rotate(-90, [0,1,0])
      translate([w / 2, -1 ,0])
      rotate(90)
        hex_wall(4, h= 1.5 + 1, rows= 4, cols= 10);
